...
class TestSelectionSort extends StatefulWidget {
  dynamic onValueChanged;

  TestSelectionSort(this.onValueChanged);

  @override
  State<TestSelectionSort> createState() => _TestSelectionSortState();
}

class _TestSelectionSortState extends State<TestSelectionSort> {
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text("Which worst case complexity does Selection Sort have?"),
                  ...
                  leading: Radio<String>(
                    ...
                    onChanged: (value) {
                      setState(() {
                        ...
                        showSolution = false;
                        answers[1] = value!;
                        isCorrect[1] = value == "It sorts by finding the smallest element and placing it first";
                      });
                    },
                  ),
                );
            ...
            if ((showSolution && isCorrect.containsKey(1) && !isCorrect[1]!) || showSolution && !isCorrect.containsKey(1))
              Text(
                "The correct answer is: It sorts by finding the smallest element and placing it first",
              ),
            const SizedBox(height: 16),

            // Question 3
            Text("Question 3", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 20)),
            Text("True or False: Selection Sort runs faster with a sorted arrays as input then with an unsorted one."),
            const SizedBox(height: 8),
            Column(
            ...
            Text("Given the following unsorted array. Which two digits are swapped first?"),
            const SizedBox(height: 8),
            Row(
              mainAxisAlignment: MainAxisAlignment.center,
              children: [4, 5, 6, 7, 3].map((num) {
                return Container(
            ...
            TextField(
              ...
              onChanged: (value) {
                setState(() {
                  ...
                  showSolution = false;
                  answers[3] = value;
                  isCorrect[3] = (value.trim().replaceAll(RegExp(r'[^0-9]'), '') == "43" || value.trim().replaceAll(RegExp(r'[^0-9]'), '') == "34");
                });
              },
            ...
            Text("The first two values in this array have already been sorted. What does the array look like after the next iteration of the Selection Sort algorithm?"),
            const SizedBox(height: 8),
            Row(
              mainAxisAlignment: MainAxisAlignment.center,
              children: [3, 5, 9, 8, 6].map((num) {
                return Container(
                  padding: const EdgeInsets.all(8.0),
                  margin: const EdgeInsets.all(4.0),
                  decoration: BoxDecoration(
                    border: Border.all(color: Colors.black),
                    borderRadius: BorderRadius.circular(4),
                    color: (num==3||num==5)?Colors.green:null,
                  ),
                  child: Text(num.toString()),
                );
              }).toList(),
            ...
            TextField(
              ...
              onChanged: (value) {
                setState(() {
                  ...
                  showSolution = false;
                  answers[4] = value;
                  isCorrect[4] = value.trim().replaceAll(RegExp(r'[^0-9]'), '') == "35689";
                });
              },
              decoration: InputDecoration(
                border: OutlineInputBorder(),
            ...
            if ((showSolution && isCorrect.containsKey(4) && !isCorrect[4]!) || showSolution && !isCorrect.containsKey(4))
              Text(
                "The correct answer is: 3 5 6 8 9",
              ),
            const SizedBox(height: 16),
